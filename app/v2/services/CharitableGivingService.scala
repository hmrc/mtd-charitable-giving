...
class CharitableGivingService @Inject()(connector: DesConnector) {
    ...
    connector.amend(amendCharitableGivingRequest).map {
      case Left(DesResponse(correlationId, MultipleErrors(errors))) =>
        ...
        if (mtdErrors.contains(DownstreamError)) {
          logger.info(
            s"[CharitableGivingService] [amend] [CorrelationId - $correlationId]" +
  ...
  def retrieve(retrieveCharitableGivingRequest: RetrieveCharitableGivingRequest)(implicit hc: HeaderCarrier,
    ...
    connector.retrieve(retrieveCharitableGivingRequest).map {
      case Left(DesResponse(correlationId, MultipleErrors(errors))) =>
        ...
        if (mtdErrors.contains(DownstreamError)) {
          logger.info(
            s"[CharitableGivingService] [retrieve] [CorrelationId - $correlationId]" +
